...
git clone https://github.com/your-repo/media-gallery-app.git
...
Usage
Video Gallery (/)
The Video Gallery is the default route, displaying a grid of video thumbnails with hoverable previews.
Features
...
Use the search bar to find videos by name or tags.
Select a file type from the dropdown to filter videos.
Scroll down to load more videos (20 at a time).
Hover over a video thumbnail to see a WebM preview.
Click a video to open the full-screen player.
Use navigation buttons or swipe gestures to browse videos.
Click "Refresh" in the header to reload video data.
...
Scroll down to load more images (16 per page).
Click an image to open a full-screen popup.
Use navigation buttons or swipe left/right to browse images.
Swipe up/down or click "Close" to exit the popup.

Styling Customization
The app uses CSS custom properties in globals.css for dynamic styling. To customize styles (e.g., column widths), you can:

Override variables in a settings component (TBD) using inline styles, e.g.:<div className="gallery-grid" style={{ "--gallery-column-width": "250px" }}>


Modify defaults in globals.css::root {
...
Use --hostname 0.0.0.0 to allow external access:npm run dev -- --hostname 0.0.0.0
...
Implement a settings component to pass dynamic props, e.g.:function Settings({ onColumnWidthChange }) {
  return (
    <input
      type="range"
      min="100"
...
Contact
For support or contributions, contact your-email@example.com or open an issue on the GitHub repository.
